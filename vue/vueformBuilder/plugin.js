...
const asyncForEach = async (array, callback) => {
  for (let index = 0; index < (_.isPlainObject(array) ? _.values(array) : array).length; index++) {
    const key = (_.isPlainObject(array) ? _.keys(array)[index] : index);
...
const getElementSchemaByPath = (schema, path) => {
  ...
  const returnSchema = (schema, level = 1) => {
    ...
    if (pathLength === level) {
      ...
      const base = schema[pathParts[level - 1]];
      const childSchema = base.type === 'list'
        ? { 0: base.element }
        : base.schema;
      ...
      setup (props, context, component) {
        ...
        const handleDrop = (e) => {
          ...
          if (path && currentPage$.value) {
            const currentPage = currentPage$.value.name;
            ...
            if (path && path.match(/\./)) {
              ...
              const to = currentPage$.value.elements?.pop();
    ...
    () => ({
      ...
      setup (props, context, component) {
        ...
        const loadSettings = async (data, section) => {
            ...
            if (field.watchers) {
              Object.keys(field.watchers).forEach((elementPath) => {
                field.watchers[elementPath].forEach((watcher) => {
                  ...
                  const watcherFunction = (value, old) => {
                    watcher[1](
                      form.el$(elementPath),
                  ...
                  const unwatch = watch(target, watcherFunction);
          ...
          const target = section ? form.el$(section) : form;
          const elements$ = section ? target.children$ : target.elements$.value;
        ...
        const openAll = () => {
          ...
          const neverOpened = Object.keys(sections.value).filter(s => onceOpened.value.indexOf(s) === -1);
    ...
    () => ({
      ...
      setup (props, context, component) {
        ...
        const handleDrop = (e, position) => {
          ...
          const page = e.dataTransfer.getData('page') || undefined;
          ...
          const path = e.dataTransfer.getData('path') || undefined;
          ...
          if (path && path.match(/\./)) {
            ...
            const to = component.form$.value.tabs$.current$.elements?.pop();
        ...
        const axios = {
          request: (options) => {
            return new Promise((resolve) => {
              ...
              return resolve(options.url == '/vueform/file/remove'
                ? true
                : {
    ...
    () => ({
      ...
      setup (props, context, component) {
        ...
        const childName = computed(() => {
          if (!element.value || !element.value?.type || !element.value?.builder?.type) {
            return;
          }
        ...
        const reset = (setDefault = true) => {
          ...
          const instances = component.form$.value.editorMode ? 1 : initial.value;
    ...
    () => ({
      ...
      setup (props, context, component) {
        ...
        const moving = computed(() => {
          ...
          const moving = component.el$.value.moving;
        ...
        const childRestrictions = computed(() => {
          const childRestrictions = getChildRestrictions(component.el$.value);
        ...
        const getChildRestrictions = (schema) => {
          ...
          const children = ['object', 'group'].indexOf(schema.type) !== -1
            ? (schema.schema || {})
            : (schema.element
                ? { 0: schema.element }
                : {}
          ...
          Object.values(children).forEach((childSchema) => {
            if (Object.values(restrictions).some(r => !r)) {
              const childRestrictions = ['object', 'group', 'list'].indexOf(childSchema.type) !== -1
        ...
        const startResizing = () => {
          const columns = component.el$.value.columnsClassesService.cols;
          let container;
          ...
          // If it does not exist, get one from the lower screen sizes
            ...
            const sizeList = [...sizes.value].reverse();
            const index = sizeList.indexOf(size.value);
        ...
        const getSiblingPath = (distance) => {
          const index = flatTree.value.indexOf(path.value);
          const sibling = flatTree.value[index + distance] || (distance > 0 ? flatTree.value[flatTree.value.length - 1] : flatTree.value[0]);
        ...
        const goToPageThatContains = (path) => {
          const page = Object.keys(component.form$.value[`${pageType.value}s`]).find((name) => {
            const page = component.form$.value[`${pageType.value}s`][name];
        ...
        const handleDragStart = function (e) {
          ...
          const schema = getElementSchemaByPath(component.form$.value.options.schema, path.value);
        ...
        const handleKeydown = (e) => {
          ...
          if (resizing.value) {
            ...
            if (e.key === 'Enter') {
              ...
              const last = component.el$.value.form$.$el.querySelector('.vfb-preview-form > div > .vfb-preview-element-container:last-of-type');
              ...
              const first = component.el$.value.form$.$el.querySelector('.vfb-preview-form > div > .vfb-preview-element-container:first-of-type');
