                ...
                <div id="inputbox">
                  ...
                  <div>
                      <input
                                ...
                                />
                          <label for="pharmakon">&nbsp;&nbsp;MBq</label>
                  </div>
                </div>
                <div id="inputbox">
                  <div id="label">
                      <label for="pollution1">Aktivität Verunreinigung 1:&nbsp;&nbsp;</label>
                  </div>
                       <div>
                        <input
                          v-model="pollution1"
                          ...
                          v-on:change="calculatePercentage"
                        />
                      <label for="pollution1">&nbsp;&nbsp;MBq</label>
                </div>
                </div>
                <div id="inputbox">
                  <div id="label">
                      <label for="pollution2">Aktivität Verunreinigung 2:&nbsp;&nbsp;</label>
                  </div>
                  <div>
                            <input
                              v-model="pollution2"
                              ...
                              v-on:change="calculatePercentage"
                            />
                          <label for="pollution2">&nbsp;&nbsp;MBq</label>
                    </div>
                </div>
              ...
              <hr>
                ...
                <div class="container" id="resultbox"><p>Die Berechnung erfolgt nach der Formel:
                 RR = (Aktivität Radiopharmakon / Gesamtaktivität) x 100</p>
                  <!--
                  ( {{fractionB }}&nbsp;MBq / ({{fractionA}}&nbsp;MBq + {{fractionB}}&nbsp;MBq + {{fractionC}}&nbsp;MBq) ) x 100
                -->
                </div>
          </div>
...
export default VueformElement({
  name: 'CartridgeElement'
}, {
  setup (props, { element }) {
    const { n } = useI18n();
    const pharmakon = ref();
    const pollution1 = ref();
    const pollution2 = ref();
    element.model.value = 0;
    const calculations = ref(0);

    const calculatePercentage = function () {
      const calc = parseFloat(pharmakon.value) / (parseFloat(pharmakon.value) + parseFloat(pollution1.value) + parseFloat(pollution2.value));
